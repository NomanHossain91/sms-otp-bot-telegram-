...
def send_welcome(message):
    try:
        ...
        # 5. بناء قائمة الأزرار الرئيسية (Main Menu)
        markup = types.InlineKeyboardMarkup(row_width=2)

        # الصف الأول: Get Number + Get Tempmail
        markup.row(
        ...
        # الصف الثاني: 2FA + Fake Name
        markup.row(
            types.InlineKeyboardButton("🔐 2FA", callback_data="menu_2fa"),
            types.InlineKeyboardButton("👤 Fake Name", callback_data="menu_fakename")
        )
        # الصف الثالث: OTHER (عريض)
        markup.add(types.InlineKeyboardButton("⬇️ OTHER", callback_data="menu_other"))
...
def handle_country_selection(call):
    try:
        ...
        # زر الجروب في الأعلى
        markup.add(types.InlineKeyboardButton("𝑉𝑖𝑒𝑤 𝑂𝑇𝑃 👀", url="https://t.me/+OQ0CTFKdFFtmZDk1"))
...
def menu_get_number(call):
    ...
    text = (
        "<b>📞 Get Number</b>\n\n"
        "<b>────────────────────</b>\n"
        "<b>Select the country you want a number from ⬇️</b>"
    )
    try:
        bot.edit_message_text(
        ...
        print(f"menu_get_number error: {e}")

# ---------- GET TEMPMAIL ----------
@bot.callback_query_handler(func=lambda call: call.data == "menu_tempmail")
def menu_tempmail(call):
    user_id = call.from_user.id
    if is_banned(user_id):
        bot.answer_callback_query(call.id, "🚫 You are banned.", show_alert=True)
        return
    bot.answer_callback_query(call.id, "⏳ Generating temp email...")
    try:
        import uuid
        random_name = uuid.uuid4().hex[:10]
        domains = ["guerrillamail.com", "tempmail.com", "sharklasers.com", "guerrillamailblock.com"]
        domain = random.choice(domains)
        email = f"{random_name}@{domain}"

        markup = types.InlineKeyboardMarkup()
        markup.row(
            types.InlineKeyboardButton("🔄 New Email", callback_data="menu_tempmail"),
            types.InlineKeyboardButton("🔙 Back", callback_data="back_to_countries")
        )
        text = (
            "<b>📧 Temp Email</b>\n\n"
            ...
            "<b>⚠️ This is a disposable email address.\n"
            "Use it for quick registrations.</b>\n"
            "<b>────────────────────</b>"
        )
        bot.edit_message_text(
            text, call.message.chat.id, call.message.message_id,
        ...
        print(f"menu_tempmail error: {e}")

# ---------- 2FA CODE GENERATOR ----------
@bot.callback_query_handler(func=lambda call: call.data == "menu_2fa")
def menu_2fa(call):
    user_id = call.from_user.id
    if is_banned(user_id):
        bot.answer_callback_query(call.id, "🚫 You are banned.", show_alert=True)
    ...
    text = (
        "<b>🔐 2FA — Two-Factor Authenticator</b>\n\n"
        "<b>────────────────────</b>\n"
        "Send me your <b>2FA secret key</b> (Base32)\n"
        "and I will generate the current OTP code for you.\n\n"
        "<b>Example key:</b> <code>JBSWY3DPEHPK3PXP</code>\n"
        "<b>────────────────────</b>"
    )
...
def handle_2fa_key(message):
    ...
    try:
        ...
        markup = types.InlineKeyboardMarkup()
        markup.row(
            types.InlineKeyboardButton("🔄 Refresh", callback_data=f"refresh_2fa_{secret}"),
            types.InlineKeyboardButton("🔙 Back", callback_data="back_to_countries")
        )
        bot.reply_to(
            message,
...
def refresh_2fa(call):
    ...
    try:
        ...
        bot.answer_callback_query(call.id, f"✅ Refreshed! Code: {otp}")
    except Exception as e:
        bot.answer_callback_query(call.id, f"❌ Error: {e}", show_alert=True)

# ---------- FAKE NAME ----------
@bot.callback_query_handler(func=lambda call: call.data == "menu_fakename")
def menu_fakename(call):
    user_id = call.from_user.id
    if is_banned(user_id):
        bot.answer_callback_query(call.id, "🚫 You are banned.", show_alert=True)
        return
    bot.answer_callback_query(call.id, "⏳ Generating identity...")

    first_names = [
        "James","John","Robert","Michael","William","David","Richard","Joseph",
        "Thomas","Charles","Emma","Olivia","Ava","Isabella","Sophia","Mia",
        "Charlotte","Amelia","Harper","Evelyn","Liam","Noah","Oliver","Elijah",
        "Lucas","Mason","Logan","Ethan","Aiden","Jackson"
    ]
    last_names = [
        "Smith","Johnson","Williams","Brown","Jones","Garcia","Miller","Davis",
        "Rodriguez","Martinez","Anderson","Taylor","Thomas","Hernandez","Moore",
        "Martin","Jackson","Thompson","White","Lopez","Wilson","Lee","Walker"
    ]
    cities = [
        "New York","Los Angeles","Chicago","Houston","Phoenix","Philadelphia",
        "San Antonio","San Diego","Dallas","San Jose","Austin","Jacksonville",
        "London","Toronto","Sydney","Melbourne","Berlin","Paris","Tokyo","Dubai"
    ]
    jobs = [
        "Software Engineer","Data Analyst","Graphic Designer","Teacher","Doctor",
        "Nurse","Accountant","Marketing Manager","Sales Representative","Architect",
        "Journalist","Photographer","Chef","Lawyer","Electrician","Mechanic"
    ]
    ...
    username_fake = f"{first.lower()}{last.lower()}{random.randint(10,99)}"
    password_fake = ''.join(random.choices("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789@#$", k=12))

    markup = types.InlineKeyboardMarkup()
    markup.row(
        types.InlineKeyboardButton("🔄 New Identity", callback_data="menu_fakename"),
        types.InlineKeyboardButton("🔙 Back", callback_data="back_to_countries")
    )
    text = (
        "<b>👤 Fake Identity Generator</b>\n\n"
        "<b>────────────────────</b>\n"
        f"<b>👤 Name:</b> <code>{first} {last}</code>\n"
        f"<b>🎂 Age:</b> <code>{age}</code>\n"
        f"<b>📅 DOB:</b> <code>{birth_day:02d}/{birth_month:02d}/{birth_year}</code>\n"
        f"<b>🏙️ City:</b> <code>{city}</code>\n"
        f"<b>💼 Job:</b> <code>{job}</code>\n"
        f"<b>🔑 Username:</b> <code>{username_fake}</code>\n"
        f"<b>🔒 Password:</b> <code>{password_fake}</code>\n"
        "<b>────────────────────</b>\n"
        "<b>⚠️ For legal use only.</b>"
    )
    try:
        bot.edit_message_text(
            text, call.message.chat.id, call.message.message_id,
        ...
        print(f"menu_fakename error: {e}")

# ---------- OTHER MENU ----------
@bot.callback_query_handler(func=lambda call: call.data == "menu_other")
def menu_other(call):
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.row(
        types.InlineKeyboardButton("💰 Balances", callback_data="other_balances"),
        types.InlineKeyboardButton("💸 Withdraw", callback_data="other_withdraw")
    )
    markup.row(
        types.InlineKeyboardButton("💬 Support", callback_data="other_support"),
        types.InlineKeyboardButton("🏠 Home", callback_data="back_to_countries")
    )
    text = (
        "<b>📋 OTHER OPTIONS</b>\n\n"
        "<b>────────────────────</b>\n"
        "<b>Select an option from below:</b>"
    )
    try:
        bot.edit_message_text(
            text, call.message.chat.id, call.message.message_id,
        ...
        print(f"menu_other error: {e}")

# ---------- BALANCES ----------
@bot.callback_query_handler(func=lambda call: call.data == "other_balances")
def other_balances(call):
    user_id = call.from_user.id
    user = get_user(user_id)
    ...
    markup.row(
        types.InlineKeyboardButton("💸 Withdraw", callback_data="other_withdraw"),
        types.InlineKeyboardButton("🔙 Back", callback_data="menu_other")
    )
    text = (
        "<b>💰 Your Balance</b>\n\n"
        "<b>────────────────────</b>\n"
        f"<b>📞 Active Number:</b> <code>{assigned_number or 'None'}</code>\n"
        f"<b>📨 OTPs Received:</b> <code>{otp_count}</code>\n"
        "<b>💵 Balance:</b> <code>$0.00</code>\n"
        "<b>────────────────────</b>\n"
        "<b>💬 Contact support to withdraw your earnings.</b>"
    )
    try:
        bot.edit_message_text(
            text, call.message.chat.id, call.message.message_id,
        ...
        print(f"other_balances error: {e}")

# ---------- WITHDRAW ----------
@bot.callback_query_handler(func=lambda call: call.data == "other_withdraw")
def other_withdraw(call):
    markup = types.InlineKeyboardMarkup()
    markup.row(
        types.InlineKeyboardButton("📩 Contact Support", url="https://t.me/nolab420"),
        types.InlineKeyboardButton("🔙 Back", callback_data="menu_other")
    )
    text = (
        "<b>💸 Withdraw</b>\n\n"
        "<b>────────────────────</b>\n"
        "<b>To request a withdrawal, please contact our support team.</b>\n\n"
        "<b>📌 Minimum withdrawal: $5.00</b>\n"
        "<b>💳 Methods: USDT • BTC • PayPal</b>\n"
        "<b>────────────────────</b>\n"
        "<b>⚡ Tap the button below to contact support.</b>"
    )
    try:
        bot.edit_message_text(
            text, call.message.chat.id, call.message.message_id,
        ...
        print(f"other_withdraw error: {e}")

# ---------- SUPPORT ----------
@bot.callback_query_handler(func=lambda call: call.data == "other_support")
def other_support(call):
    markup = types.InlineKeyboardMarkup()
    markup.row(
        types.InlineKeyboardButton("📩 Contact Owner", url="https://t.me/nolab420"),
        types.InlineKeyboardButton("📢 Channel", url="https://t.me/+OQ0CTFKdFFtmZDk1")
    )
    markup.add(types.InlineKeyboardButton("🔙 Back", callback_data="menu_other"))
    text = (
        "<b>💬 Support</b>\n\n"
        "<b>────────────────────</b>\n"
        "<b>Need help? We are here for you!</b>\n\n"
        "<b>👨‍💻 Owner:</b> @nolab420\n"
        "<b>📢 Channel:</b> PocketMoneyHub\n"
        "<b>⏰ Response time:</b> Usually within 24h\n"
        "<b>────────────────────</b>"
    )
    try:
        bot.edit_message_text(
