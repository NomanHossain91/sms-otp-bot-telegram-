...
def back_to_main_menu(call):
    try:
        ...
        # শুধু main keyboard দেখাও, welcome message পাঠাবে না
        bot.send_message(
            call.message.chat.id,
            "🏠 <b>Main Menu</b>",
            parse_mode="HTML",
...
def handle_no_numbers(call):
    bot.answer_callback_query(call.id, "❌ এখন কোনো নম্বর নেই, পরে চেষ্টা করুন।", show_alert=True)

@bot.callback_query_handler(func=lambda call: call.data.startswith("ivasms_country_"))
def handle_ivasms_country(call):
    try:
        user_id = call.from_user.id
        ...
        cc = call.data.replace("ivasms_country_", "")

        # iVasms se fresh numbers fetch karo
        ivasms_numbers = fetch_ivasms_available_numbers()
        available = ivasms_numbers.get(cc, [])

        if not available:
            bot.answer_callback_query(call.id, "❌ এই দেশের কোনো নম্বর নেই।", show_alert=True)
            return

        # Old number release karo
        old_user = get_user(user_id)
        if old_user and old_user[5]:
            release_number(old_user[5])

        # Random number assign karo
        assigned = random.choice(available)
        assign_number_to_user(user_id, assigned)
        save_user(user_id, country_code=cc, assigned_number=assigned)
        ...
        print(f"❌ handle_ivasms_country error: {e}")
        traceback.print_exc()
...
# ---------- iVasms se live numbers fetch karo ----------
def fetch_ivasms_available_numbers():
    """
    iVasms dashboard se available numbers fetch karo.
    Returns: dict { country_code: [number1, number2, ...] }
    """
    ...
    try:
        ...
        summary_resp = session.post(sms_api_url, headers=headers, data=summary_payload, timeout=15)
        ...
        for group_id in group_ids:
            ...
            numbers_resp = session.post(numbers_url, headers=headers, data=numbers_payload, timeout=15)
            numbers_soup = BeautifulSoup(numbers_resp.text, 'html.parser')
            number_divs = numbers_soup.select("div[onclick*='getDetialsNumber']")
            phone_numbers = [clean_number(div.text.strip()) for div in number_divs if div.text.strip()]

            for phone in phone_numbers:
                # country detect karo number se
                _, _, cc = get_country_info(phone)
                if not cc or cc == "??":
                    cc = "UNKNOWN"
                result.setdefault(cc, [])
                if phone not in result[cc]:
                    result[cc].append(phone)

        print(f"[iVasms] ✅ Available numbers fetched: {result}")
        return result

    except Exception as e:
        print(f"[iVasms] ❌ fetch_ivasms_available_numbers error: {e}")
...
def build_country_markup(user_id):
    ...
    if ivasms_numbers:
        for cc, nums in ivasms_numbers.items():
            if cc in COUNTRY_CODES:
                name, flag, _ = COUNTRY_CODES[cc]
            else:
                name, flag = cc, "🌍"
            count = len(nums)
            btn_text = f"{flag} {name} ({count})"
            buttons.append(types.InlineKeyboardButton(btn_text, callback_data=f"ivasms_country_{cc}"))
    else:
        # fallback: DB combos
        user_data = get_user(user_id)
    ...
    if not buttons:
        buttons.append(types.InlineKeyboardButton("❌ কোনো নম্বর নেই", callback_data="no_numbers"))
...
def handle_get_number(message):
    ...
    bot.send_message(
        message.chat.id,
        "🌍 নিচে থেকে আপনার পছন্দের দেশটি সেলেক্ট করুন:",
        reply_markup=markup
    )
...
@bot.message_handler(func=lambda msg: msg.text == "◀️ Back Menu")
def handle_back_menu_button(message):
    """Fake Name সহ যেকোনো জায়গা থেকে Back Menu press করলে main menu তে যাবে"""
    user_id = message.from_user.id
    if user_id in user_states:
        del user_states[user_id]
    bot.send_message(
