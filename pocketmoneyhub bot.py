    ...
    try:
        ...
        try:
            bot.send_message(
                ...
                "<b>☎️ Get Number</b>\n\n<b>নিচে থেকে একটি দেশ সিলেক্ট করুন ⬇️</b>",
                parse_mode="HTML",
                reply_markup=build_country_markup(call.from_user.id)
            )
        except:
            pass
...
@bot.message_handler(func=lambda msg: msg.text == "◀️ Back Menu" and user_states.get(msg.from_user.id, "").startswith("fake_country_"))
def handle_back_from_fake_result(message):
    user_id = message.from_user.id
    if user_id in user_states:
        del user_states[user_id]
    bot.send_message(
